-- Northwind MySQL Database Script
-- Reference: instnwnd.sql (SQL Server version)
-- Tables: Categories, Shippers, CustomerDemographics, CustomerCustomerDemo, Customers
-- Note: Categories table moved from SQLite to MySQL due to hex literal size limitations in SQLite
...
CREATE INDEX idx_Shippers_CompanyName ON Shippers(CompanyName);
...
CREATE INDEX idx_CustomerCustomerDemo_CustomerID ON CustomerCustomerDemo(CustomerID);
CREATE INDEX idx_CustomerCustomerDemo_CustomerTypeID ON CustomerCustomerDemo(CustomerTypeID);
